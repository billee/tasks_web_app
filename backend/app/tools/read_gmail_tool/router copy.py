...
async def read_inbox_endpoint(
    ...
    try:
        ...
        if not service:
            ...
            auth_url = client.get_auth_url(current_user.id)
            return {
                "success": False,
        ...
        if e.status_code == 401 and e.detail and isinstance(e.detail, dict) and 'auth_url' in e.detail:
            # OAuth required
            return {
                "success": False,
                "message": "Gmail authentication required. Click the button below to authorize access to your Gmail account.",
                "tool_results": [
                    {
                        ...
                        "button_text": e.detail.get('button_text', 'Authorize Gmail Access')
                    }
                ]
            }
        raise e
